...
class DB
{
    ...
    {
        $this->parameters[sizeof($this->parameters)] = array(":" . $para , $value);//[":" . $para , $value];
    }
    /**
     *	@void
     *	
    ...
    public function insert($tablename,$datas, $params = null, $fetchmode = PDO::FETCH_ASSOC)
    {
        ...
        foreach ($datas as $key => $value) {
            $datas_ins[$key] = str_replace("'", "\'", trim($value));
        }

        $query = 'insert into '.$tablename.' ';
        $query .= ' ('.implode(",", array_keys($datas_ins)).') ';
        $query .= ' values (\''.implode("','", array_values($datas_ins)).'\') ';
